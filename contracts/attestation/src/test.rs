...
impl TestEnv {
    ...
    pub fn submit_attestation(&self, business: Address, period: String, merkle_root: BytesN<32>, timestamp: u64, version: u32) {
        self.client.submit_attestation(&business, &period, &merkle_root, &timestamp, &version);
    }

    pub fn revoke_attestation(&self, caller: Address, business: Address, period: String, reason: String) {
        self.client.revoke_attestation(&caller, &business, &period, &reason);
    }
    ...
    pub fn get_attestation(&self, business: Address, period: String) -> Option<(BytesN<32>, u64, u32, i128)> {
        self.client.get_attestation(&business, &period)
    }

    pub fn get_attestation_with_status(&self, business: Address, period: String) -> Option<((BytesN<32>, u64, u32, i128), Option<(Address, u64, String)>)> {
        self.client.get_attestation_with_status(&business, &period)
    }

    pub fn verify_attestation(&self, business: Address, period: String, merkle_root: &BytesN<32>) -> bool {
        self.client.verify_attestation(&business, &period, merkle_root)
    }

    pub fn get_business_attestations(&self, business: Address, periods: Vec<String>) -> Vec<(String, Option<(BytesN<32>, u64, u32, i128)>, Option<(Address, u64, String)>)> {
